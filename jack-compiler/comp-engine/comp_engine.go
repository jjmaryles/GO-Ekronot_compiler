...
import (
	...
	"GO-Ekronot_compiler/jack-compiler/mapper"
	st "GO-Ekronot_compiler/jack-compiler/symbol-table"
	"GO-Ekronot_compiler/jack-compiler/tokenizer"
	vld "GO-Ekronot_compiler/jack-compiler/validators"
	vm "GO-Ekronot_compiler/jack-compiler/vm-writer"
	"os"
	"strconv"
)
...
func Compile(inFile, outFile string) {
	...
	utilities.HandleErr(err)                         // handle possible error
...
func (e *Engine) compileParameterList() {
	...
	if e.subroutineKind == "method" {
		e.table.Define("this", e.className, mapper.IdentifierTypeArg)
	}
	...
	for {
		...
		e.table.Define(varName, varType, mapper.IdentifierTypeArg)
...
func (e *Engine) compileSubroutineBody() {
	...
	if e.subroutineKind == "constructor" {
		vm.WritePush(e.outFile, mapper.SegmentCONST, e.table.VarCount("field"))
		vm.WriteCall(e.outFile, "Memory", "alloc", 1)
		vm.WritePop(e.outFile, mapper.SegmentPOINT, 0)
	}
	if e.subroutineKind == "method" {
		vm.WritePush(e.outFile, mapper.SegmentARG, 0)
		vm.WritePop(e.outFile, mapper.SegmentPOINT, 0)
	}
...
func (e *Engine) compileVarDec() {
	...
	for {
		varName := e.eatType(tokenizer.TokenTypeIdentifier) // varName
		e.table.Define(varName, varType, mapper.IdentifierTypeVar)
		if e.currentToken().S != "," {
...
func (e *Engine) compileReturnStatement() {
	...
	if e.currentToken().S != ";" {
		...
		vm.WritePush(e.outFile, mapper.SegmentCONST, 0) // void
	}
...
func (e *Engine) compileWhileStatement() {
	...
	vm.WriteArithmetic(e.outFile, mapper.ArithmCmdNOT)
...
func (e *Engine) compileLetStatement() {
	...
	if isArray {
		...
		vm.WriteArithmetic(e.outFile, mapper.ArithmCmdADD)
	}
	...
	if isArray {
		vm.WritePop(e.outFile, mapper.SegmentTEMP, 0)
		vm.WritePop(e.outFile, mapper.SegmentPOINT, 1)
		vm.WritePush(e.outFile, mapper.SegmentTEMP, 0)
		vm.WritePop(e.outFile, mapper.SegmentTHAT, 0)
	} else {
...
func (e *Engine) compileSubroutineCall() {
	...
	if e.currentToken().S == "." {
		...
		funcName = identifier
		nArgs++
		vm.WritePush(e.outFile, mapper.SegmentPOINT, 0)
	}
...
func (e *Engine) compileExpression() {
	...
	for {
		...
		cmd, _ := mapper.ArithmSymbols[e.currentToken().S]
...
func (e *Engine) compileTerm() {
	...
	if next.T == tokenizer.TokenTypeIntConstant {
		...
		if next.T == tokenizer.TokenTypeSymbol && next.S == "[" {
			...
			vm.WriteArithmetic(e.outFile, mapper.ArithmCmdADD)
			vm.WritePop(e.outFile, mapper.SegmentPOINT, 1)
			vm.WritePush(e.outFile, mapper.SegmentTHAT, 0)
		}
	}
}
